...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserDO> implements UserService {
    ...
    public UserRespDTO getUserByUsername(String username) {
        // 先查数据库，没有抛异常，有则返回。
        // TODO 多个请求刷新造成数据库宕机怎么办，改成redis怎么改
        // TODO 端口问题 本地是能用8002 但是要怎么阻止外面使用8002端口，拦截器？
    ...
    public void update(UserUpdateReqDTO requestParam) {
        if (!requestParam.getUsername().equals(UserContext.getUsername()))
            throw new ClientException("当前用户信息与尝试修改用户信息不一致");
    ...
    public UserLoginRespDTO login(UserLoginReqDTO requestParam) {
        // 查询用户是否存在且没注销
        // 生成token
        // TODO login页面登录后还能进。发现应该跟前端联手或者使用MVC，如果用MVC会不会和filter重复
        // TODO 用jwt？
        ...
        // TODO 重复登陆后会带token，这个token有什么用。
        if (CollUtil.isNotEmpty(hasLoginMap)) {
            stringRedisTemplate.expire(USER_LOGIN_KEY + requestParam.getUsername(), 3000L, TimeUnit.MINUTES);
